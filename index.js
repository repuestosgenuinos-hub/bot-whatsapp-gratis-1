...
// ============================================================
// MOTOR DE BÚSQUEDA ULTRA-FLEXIBLE (SISTEMA DE RELEVANCIA)
// ============================================================
async function buscarProductoPorTexto(texto) {
    const txtNormal = normalizar(texto);
    
    // DICCIIONARIO EXTENDIDO DE PALABRAS BASURA (STOPWORDS)
    const stopWords = [
        'hola', 'saludos', 'buenos', 'buenas', 'dias', 'tardes', 'noches', 'como', 'estas', 'esta',
        'tienes', 'tendra', 'tendras', 'tendria', 'tienen', 'tengas', 'tenga', 'tendrias', 'tendrias',
        'busco', 'buscando', 'queria', 'quería', 'quisiera', 'necesito', 'podrias', 'podrías', 'puedes', 'puedo',
        'la', 'del', 'de', 'el', 'una', 'un', 'hay', 'si', 'es', 'con', 'para', 'por', 'en', 'al', 'lo', 'los', 'las', 'una',
        'precio', 'cuanto', 'mide', 'venden', 'donde', 'estos', 'estos', 'estos', 'este', 'esta', 'esto',
        'gracias', 'favor', 'ayuda', 'cordial', 'muchas', 'todo', 'bienvenidos', 'bendiciones',
        'familia', 'espero', 'encuentres', 'bien', 'preguntarte', 'saludo', 'estimados', 'estimado',
        'buen', 'buena', 'bueno', 'se', 'me', 'le', 'te', 'su', 'sus', 'mi', 'mis', 'tu', 'tus',
        'que', 'cual', 'quien', 'cuando', 'porque', 'pues', 'pero', 'mas', 'muy', 'asi', 'aun', 'entre', 'sin',
        'sobre', 'tras', 'durante', 'mediante', 'excepto', 'segun', 'hacer', 'hace', 'ser', 'estar', 'tener',
        'tengo', 'tenemos', 'tiene', 'decir', 'dice', 'ver', 'veo', 'vez', 'veces', 'quiero', 'quiere', 
        'gustaria', 'gusta', 'necesita', 'pueda', 'unid', 'unidades', 'unidad',
        'listo', 'claro', 'ok', 'okey', 'vale', 'va', 'vamos', 'vaya', 'algun', 'ningun', 'tipo', 'tipos',
        'disculpa', 'disculpe', 'permiso', 'ayudar', 'apoyo', 'consulta', 'consultar', 'info', 'informacion', 'decirme', 'dime',
        'avísame', 'avisa', 'saber', 'sabes', 'saben', 'sabemos', 'pana', 'panas', 'brother', 'bro', 'amigo', 'amigos',
        'estoy', 'estaba', 'estaban', 'vengo', 'vienes', 'viene', 'vienen', 'voy', 'vas', 'va', 'iban', 'llegando', 'pais', 'atento'
    ];
    ...
    if (esMedida) {
        // Preservamos la medida tal cual y también sin espacios para cubrir ambos casos
        palabrasBase = [txtNormal, txtNormal.replace(/\s+/g, '')];
    } else {
        // Filtramos palabras basura y dejamos solo las palabras clave (ej. "rolinera", "trasera", "fiesta")
    ...
    try {
        // Construimos una query de RELEVANCIA. 
        // No buscamos que tenga TODAS las palabras, sino que traiga los que tengan AL MENOS UNA,
        // pero ordenando primero los que tengan más coincidencias.
        let relevanceSql = "";
        let whereClause = "";
        let params = [];

        palabrasBase.forEach((pal, index) => {
            // Sumamos 1 punto por cada coincidencia en cualquiera de los 3 campos
        ...
        // Para que el ORDER BY funcione, necesitamos pasarle los mismos parámetros otra vez
        ...
        console.log("Error en motor de búsqueda:", e);
    }
...
async function checkFacturasVencidas() {
    ...
    try {
        ...
        for (const f of facturas) {
            const nivel = obtenerNivelRecordatorio(f.dias_vencida);
            if (!nivel) continue;
            const monto = (parseFloat(f.total) - parseFloat(f.abono_factura || 0)) / (parseFloat(f.porcentaje) || 1);
            if (monto <= 0) continue;
            const fecha = new Date(f.fecha_reg).toISOString().split('T')[0];
            if (!(enviados[f.id_factura] && enviados[f.id_factura].includes(nivel))) {
                const jid = formatWhatsApp(f.celular);
                if (jid) await safeSendMessage(jid, { text: obtenerTonoMensaje(nivel, f, monto, fecha) });
...
async function checkVendedoresRecordatorio() {
    ...
    try {
        ...
        if (ultimo && Math.floor((new Date() - new Date(ultimo)) / 86400000) < 3) return;
        ...
        for (const key of Object.keys(vendedoresMap)) {
            ...
            if (v.jid) await safeSendMessage(v.jid, { text: `📢 *RESUMEN DE CLIENTES VENCIDOS*\n\nVendedor: *${v.nombre}*\n\n${v.facturas.join('\n')}\n\nLe recordamos la importancia de gestionar estos cobros.` });
            await sleep(1000);
        }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        try {
            ...
            if (msg.key.fromMe) {
                ...
                if (textMe === '!bot') { await setModo(from, 'bot'); await safeSendMessage(from, { text: "🤖 Bot reactivado." }); } 
            ...
            // --- 1. PRIORIDAD: ENRUTAMIENTO RIF (SÓLO ADMINS) ---
            if (isAdmin) {
            ...
            // --- 2. BÚSQUEDA DE PRODUCTOS (SOPORTE TOTAL PARA TODOS) ---
            ...
            // --- 4. PAGOS / FACTURAS FISCALES ---
            if (text.includes('pago') || text.includes('abono') || text.includes('envié el pago') || text.includes('adjunto pago')) {
            ...
            if (text.includes('factura fiscal') || text.includes('factura con iva')) {
                const nombreUsuario = vendedor ? vendedor.nombre : pushName;
            ...
            if (conversationalShorts.includes(text) || rawText.length > 500) return;
        ...
        } catch (e) { console.log("[MSG] Error en handler:", e.message); }
    });
}
